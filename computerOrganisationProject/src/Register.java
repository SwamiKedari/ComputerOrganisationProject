...
public class Register {
    ...
    public void breakMemoryInstruction(String a) {
        ...
        //System.out.println("Net Instruction no");
        System.out.println();
        String[] c=b[0].split(" ");
        ...
        if(c[0].charAt(k)==':'&&c[1].equals(".byte")){
            ...
            MEM[memoryPointer]=Integer.parseInt(c[2]);}
                ...
                System.err.println("Syntax error !");
                System.err.println("Incorrect DataType provided . ");
                System.exit(0);
            }
    ...
    public void addToMemory(ArrayList<String> a){
        ...
        for(int k=1;k<a.size()&&!a.get(k).equals(".text");k++){
            if((!a.get(k).equals(".text"))&&a.get(k).charAt(a.get(k).length()-1)==':'){
                System.err.println("Syntax error !");
                System.exit(0);
            }
    ...
    */
    ...
    public ArrayList<Integer> FetchRegister(String s)
    {
        ...
        if(Itype <= 2)
        {
            ...
            for(int i=0; i<3; i++)
            {
                ...
                int n=0;
                try{
                n = Integer.parseInt(reg[i].substring(1));}
                catch(Exception e){
                    System.err.println("Syntax error at Instruction no :"+(parser.PC+1));
        ...
        else if(Itype==5 || Itype==6)
        {
            ...
            int n=0;
            try{
                n = Integer.parseInt(reg[0].substring(1));}
            catch(Exception e){
                System.err.println("Syntax error at Instruction no :"+(parser.PC+1));
                System.err.println("Invalid Register .");
                System.exit(0);
            ...
            if(r2.charAt(0)!='x'){
                System.err.println("Syntax error at instruction : "+(parser.PC+1));
                System.err.println("Wrong register name.");
                return r0;
            }
        ...
        else if(Itype==12)
        {
            ...
            if(s.equals("ra")){
                ...
                System.err.println("Invalid Instruction at Instruction no : " +(parser.PC+1));
                return r0;
            }
        }
        else if(Itype==13){
    ...
    {
        R[0]=0; //since the zero register has the value zero always .
        if(Itype==1)
        {
        ...
        else if(Itype==5)
        {
            try {
                ...
                System.err.println("IndexOutOfBoundException at Instruction no: "+ (parser.PC+1));
                parser.PC = -1;
            }
        }
        else if(Itype==6)
        ...
        else if(Itype==7)
        {
            try {
                ...
                System.err.println("Syntex error at Instruction no: "+ (parser.PC+1));
                parser.PC = -1;
            }
        }
        else if(Itype==8)
